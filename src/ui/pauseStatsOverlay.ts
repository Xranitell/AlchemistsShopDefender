...
export class PauseStatsOverlay {
  ...
  private uniqueEffects(state: GameState): StatLine[] {
    ...
    if (m.towerBonusVsBurning) lines.push({ label: tWithFallback('ui.pause.effect.crossfire', '+30% к урону горящим'), desc: tWithFallback('ui.pause.effect.crossfire.desc', 'Стойки наносят +30% к урону горящим врагам'), value: '●', kind: 'unique' });
    ...
    if (m.thornyShell) lines.push({ label: tWithFallback('ui.pause.effect.thorns', 'Шипастая оболочка'), desc: tWithFallback('ui.pause.effect.thorns.desc', 'Враги получают 8 ответного урона при касании Манекена'), value: '●', kind: 'unique' });
    if (m.tripleThrowActive) lines.push({ label: tWithFallback('ui.pause.effect.triple', 'Тройной бросок'), desc: tWithFallback('ui.pause.effect.triple.desc', 'Веер из 3 склянок каждые 8 секунд'), value: `${m.tripleThrowInterval}с`, kind: 'unique' });
    ...
    if (m.archmasterActive) lines.push({ label: tWithFallback('ui.pause.effect.archmaster', 'Архимастер'), desc: tWithFallback('ui.pause.effect.archmaster.desc', 'Новые стойки строятся минимум 2 уровня'), value: '●', kind: 'unique' });
    if (m.fireRubyActive || m.fireRubyCounter > 0) lines.push({ label: tWithFallback('ui.pause.effect.fireRuby', 'Огненный рубин'), desc: tWithFallback('ui.pause.effect.fireRuby.desc', 'Каждая 5-я склянка поджигает и оставляет огненную лужу'), value: `${m.fireRubyCounter}`, kind: 'unique' });
    if (m.mercuryRingActive) lines.push({ label: tWithFallback('ui.pause.effect.mercRing', 'Ртутный обруч'), desc: tWithFallback('ui.pause.effect.mercRing.desc', 'Враги рядом с Манекеном движутся на 40% медленнее'), value: '●', kind: 'unique' });
    if (m.aetherEngineActive) lines.push({ label: tWithFallback('ui.pause.effect.aetherEngine', 'Эфирный двигатель'), desc: tWithFallback('ui.pause.effect.aetherEngine.desc', '+15 к заряду Перегрузки за каждую реакцию'), value: '●', kind: 'unique' });
