...
export class PauseStatsOverlay {
  ...
  private uniqueEffects(state: GameState): StatLine[] {
    ...
    if (m.towerBonusVsBurning) lines.push({ label: tWithFallback('ui.pause.effect.crossfire', '+30% урона горящим'), desc: tWithFallback('ui.pause.effect.crossfire.desc', 'Стойки наносят +30% к урону горящим врагам'), value: '●', kind: 'unique' });
